...
def main():
    ...
    if command == "init":
        ...
        run_command(f'alembic revision --autogenerate -m "{message}"', "Creando migración")
        
    elif command == "upgrade":
        # Aplicar migraciones
        revision = sys.argv[2] if len(sys.argv) > 2 else "head"
        run_command(f"alembic upgrade {revision}", f"Aplicando migraciones hasta {revision}")
        
    elif command == "downgrade":
        # Revertir migración
        revision = sys.argv[2] if len(sys.argv) > 2 else "-1"
        run_command(f"alembic downgrade {revision}", f"Revirtiendo migración a {revision}")
        ...
        revision = sys.argv[2] if len(sys.argv) > 2 else "head"
        run_command(f"alembic stamp {revision}", f"Marcando {revision} como aplicada")
