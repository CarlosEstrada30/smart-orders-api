        ...
        with engine.connect() as connection:
            for sql in enum_values:
                try:
                    ...
                    print(f"⚠️ Enum value issue (might be expected): {enum_error}")
...
def verify_columns(engine):
    ...
    try:
        with engine.connect() as connection:
            ...
            expected_columns = {
                'dte_number', 'fel_authorization_date', 'fel_certification_date', 
                'fel_certifier', 'fel_error_message', 'fel_number', 'fel_series', 
                'fel_uuid', 'fel_xml_path', 'requires_fel'
            }
...
if __name__ == "__main__":
    ...
    parser.add_argument('--show-enum', action='store_true', help='Show current enum values')
