...
class MigrationStatusChecker:
    ...
    def get_tenant_info(self) -> List[Dict[str, str]]:
        ...
        try:
            ...
            tenants = self.tenant_service.get_tenants(db, limit=1000, include_inactive=True)
    ...
    def generate_status_report(self) -> None:
        ...
        print(f"   - En trial: {sum(1 for t in tenants_info if t['is_trial'] == 'Sí')}")
        ...
        if all_tenant_schemas:
            ...
            print("-" * 85)
            print(f"{'#':<3} {'Schema':<35} {'Estado Migración':<25} {'Existe en tabla tenants':<20}")
            print("-" * 85)
            ...
            for i, schema_name in enumerate(all_tenant_schemas, 1):
                ...
                print(f"{i:<3} {schema_name[:34]:<35} {migration_status:<25} {is_registered:<20}")
        else:
        ...
        if inactive_tenants:
            ...
            for tenant in inactive_tenants[:5]:  # Mostrar solo los primeros 5
                print(f"   - {tenant['nombre']} ({tenant['schema_name']}) - Creado: {tenant['created_at']}")
        ...
        all_tenant_schemas = self.get_all_tenant_schemas()
        tenant_schemas_registered = {t['schema_name'] for t in tenants_info}
        orphan_schemas = [s for s in all_tenant_schemas if s not in tenant_schemas_registered]
