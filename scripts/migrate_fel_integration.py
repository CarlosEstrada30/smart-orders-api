            ...
            with connection.begin():
                for i, sql in enumerate(migration_sql, 1):
                    ...
                    try:
                        ...
                        print(f"⚠️ Step {i} had an issue (might be expected): {step_error}")
...
def verify_migration(engine):
    ...
    try:
        with engine.connect() as connection:
            ...
            expected_columns = {
                'fel_authorization_date', 'fel_certification_date', 'fel_certifier',
                'fel_error_message', 'fel_number', 'fel_series', 'fel_uuid',
                'fel_xml_path', 'dte_number', 'requires_fel'
            }
            ...
            if expected_columns.issubset(found_columns):
                ...
                for column_name, data_type, nullable in columns:
                    print(f"   - {column_name}: {data_type} ({'nullable' if nullable == 'YES' else 'not null'})")
            else:
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description='FEL Integration Migration for Guatemala')
