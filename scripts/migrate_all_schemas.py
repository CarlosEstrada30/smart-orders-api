...
class MultiTenantMigrator:
    ...
    def get_all_tenant_schemas(self) -> List[Tuple[str, str]]:
        ...
        try:
            ...
            logger.error(
                f"Error obteniendo schemas de la base de datos: {str(e)}")
    ...
    def get_migration_status(self, schema_name: str) -> str:
        ...
        try:
            ...
            with connection_engine.connect() as connection:
                # Primero verificar si la tabla alembic_version existe en el
                # schema específico
                result = connection.execute(text("""
                    SELECT EXISTS (
                ...
                # Si existe la tabla, obtener la versión usando search_path
                # específico
                if schema_name != "public":
                    connection.execute(
    ...
    def _create_base_tables_for_schema(self, schema_name: str) -> None:
        ...
        try:
            ...
            raise Exception(
                f"Error creando tablas base para schema {schema_name}: {str(e)}")
    ...
    def migrate_all_schemas(self) -> None:
        ...
        if not tenant_schemas:
            ...
            print(
                f"   📊 Se encontraron {len(tenant_schemas)} schemas de tenants")

        # 3. Migrar cada schema de tenant
        for i, (schema_display_name, schema_name) in enumerate(
                tenant_schemas, 1):
            print(
            ...
            # Si no hay alembic_version o está vacío, crear las tablas base
            # primero
    ...
    def print_summary(self) -> None:
        ...
        if failed > 0 or warnings > 0:
            ...
            for result in self.results:
                if result["status"] != "✅":
                    print(
                        f"   {result['status']} {result['tenant']} ({result['schema']}): {result['message']}")
