        ...
        if response.status_code == 200:
            ...
            if orders_response.status_code == 200:
                ...
                for i, order in enumerate(orders, 1):
                    ...
                    if client:
                        print(
                            f"   👤 Cliente: {client.get('name')} ({client.get('email')})")
                    ...
                    for j, item in enumerate(items, 1):
                        ...
                        print(
                            f"      {j}. {product_name or 'Sin nombre'} (SKU: {product_sku or 'Sin SKU'})")
                        print(
                            f"         Cantidad: {quantity}, Precio: ${unit_price:.2f}, Total: ${total_price:.2f}")

                        if not product_name:
                            print(
                                f"         ⚠️  Producto sin nombre (ID: {item.get('product_id')})")
                ...
                if orders:
                    ...
                    specific_order_response = requests.get(
                        f"{ORDERS_URL}/{order_id}", headers=headers)

                    if specific_order_response.status_code == 200:
                        specific_order = specific_order_response.json()
                        print(f"✅ Orden específica obtenida")
                        print(
                            f"   📝 Número: {specific_order.get('order_number')}")
                        print(
                            f"   👤 Cliente: {specific_order.get('client', {}).get('name', 'No encontrado')}")
                        print(
                            f"   📦 Items: {len(specific_order.get('items', []))}")
                    else:
                        print(
                            f"❌ Error al obtener orden específica: {specific_order_response.status_code}")

                # 5. Probar filtro por estado
                print(f"\n5️⃣ Probando filtro por estado...")

                status_filter_response = requests.get(
                    f"{ORDERS_URL}/?status_filter=pending", headers=headers)

                if status_filter_response.status_code == 200:
                    pending_orders = status_filter_response.json()
                    print(
                        f"✅ Órdenes pendientes obtenidas: {len(pending_orders)}")
                else:
                    print(
                        f"❌ Error al filtrar por estado: {status_filter_response.status_code}")

            else:
                print(
                    f"❌ Error al obtener órdenes: {orders_response.status_code}")
