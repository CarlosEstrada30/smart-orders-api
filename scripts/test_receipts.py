...
class ReceiptTester:
        ...
        if response.status_code == 200:
            ...
            if orders:
                ...
                for i, order in enumerate(
                        orders[:3]):  # Mostrar solo las primeras 3
                    print(
                        f"   {i+1}. {order['order_number']} - {order.get('client', {}).get('name', 'N/A')} - Q {order['total_amount']:,.2f} ({order['status']})")
                return orders
    ...
    def test_receipt_download(self, order_id):
        ...
        if response.status_code == 200:
            ...
            print(
                f"✅ Comprobante descargado: {filename} ({len(response.content)} bytes)")
            return filename
        else:
    ...
    def test_receipt_preview(self, order_id):
        ...
        response = self.session.get(
            f"{BASE_URL}/orders/{order_id}/receipt/preview")

        if response.status_code == 200:
            print(
                f"✅ Preview funcionando - Tamaño: {len(response.content)} bytes")
            print(
                f"   URL: http://localhost:8000/api/v1/orders/{order_id}/receipt/preview")
            return True
        else:
    ...
    def test_receipt_generation(self, order_id):
        ...
        response = self.session.post(
            f"{BASE_URL}/orders/{order_id}/receipt/generate")
    ...
    def test_order_details(self, order_id):
        ...
        if response.status_code == 200:
            ...
            print(
                f"   👤 Cliente: {order.get('client', {}).get('name', 'N/A')}")
    ...
    def run_complete_test(self):
        ...
        print(
            f"   👀 Preview: http://localhost:8000/api/v1/orders/{order_id}/receipt/preview")
        print(
            f"   ⬇️ Descarga: http://localhost:8000/api/v1/orders/{order_id}/receipt")
