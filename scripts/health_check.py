...
async def check_migrations_status() -> Dict[str, Any]:
    ...
    try:
        ...
        if public_status and not public_status.startswith("ERROR"):
            return {"status": "healthy", "message": f"Migrations OK - Version: {public_status}"}
        else:
            return {"status": "warning", "message": f"Migration status: {public_status}"}
    
    except Exception as e:
        return {"status": "warning", "message": f"Migration check error: {str(e)}"}
...
async def check_environment_variables() -> Dict[str, Any]:
    ...
    try:
        ...
        return {"status": "unhealthy", "message": f"Environment check error: {str(e)}"}
