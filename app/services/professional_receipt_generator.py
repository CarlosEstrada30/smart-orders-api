...
class ProfessionalReceiptGenerator:
    ...
    def _create_header(self, settings: Settings):
        ...
        if logo_cell:
            header_data = [[logo_cell, Paragraph(company_info, self.styles['Normal'])]]
    ...
    def _get_logo_image(self, logo_url: str, max_width=3*cm, max_height=3*cm):
    ...
    def _create_order_info(self, order: Order):
        ...
        elements.append(Paragraph("INFORMACIÓN DEL PEDIDO", self.styles['SectionTitle']))
        ...
        if order.updated_at and order.updated_at != order.created_at:
            order_data.append(["Última Actualización:", order.updated_at.strftime('%d/%m/%Y %I:%M %p')])
        ...
        if order.notes:
            elements.append(Spacer(1, 5*mm))
            elements.append(Paragraph("<b>Notas del Pedido:</b>", self.styles['ImportantInfo']))
    ...
    def _create_products_table(self, order: Order):
        ...
        # Headers de la tabla
        headers = ['#', 'Producto', 'Descripción', 'Cantidad', 'Precio Unit.', 'Total']
    ...
    def _create_footer(self, settings: Settings):
        ...
        # Notas importantes
        elements.append(Paragraph("INFORMACIÓN IMPORTANTE", self.styles['SectionTitle']))
        ...
        elements.append(Paragraph(f'<para align="center"><i>{thank_you}</i></para>', self.styles['CompanyInfo']))
        
        # Timestamp de generación
        timestamp = f"Comprobante generado el {datetime.now().strftime('%d de %B de %Y a las %I:%M %p')}"
        elements.append(Paragraph(f'<para align="center">{timestamp}</para>', self.styles['CompanyInfo']))
        ...
        if settings.phone or settings.email:
            ...
            elements.append(Paragraph(f'<para align="center">Para consultas: {contact_text}</para>', self.styles['CompanyInfo']))
