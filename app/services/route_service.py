...
class RouteService:
    ...
    def get_active_routes(self, db: Session, skip: int = 0, limit: int = 100) -> List[Route]:
    ...
    def update_route(self, db: Session, route_id: int, route_update: RouteUpdate) -> Optional[Route]:
        ...
        if "name" in update_data:
            existing_route = self.repository.get_by_name(db, name=update_data["name"])
    ...
    def delete_route(self, db: Session, route_id: int) -> Optional[Route]:
        ...
        return self.repository.update(db, db_obj=db_route, obj_in={"is_active": False})
    ...
    def reactivate_route(self, db: Session, route_id: int) -> Optional[Route]:
        ...
        return self.repository.update(db, db_obj=db_route, obj_in={"is_active": True})
