...
class ProductService:
    ...
    def update_product(self, db: Session, product_id: int, product_update: ProductUpdate) -> Optional[Product]:
        ...
        if "sku" in update_data:
            existing_product = self.repository.get_by_sku(db, sku=update_data["sku"])
            if existing_product and existing_product.id != product_id:
                raise ValueError("Product with this SKU already exists")
        
        return self.repository.update(db, db_obj=db_product, obj_in=update_data)
    ...
    def delete_product(self, db: Session, product_id: int) -> Optional[Product]:
        ...
        return self.repository.update(db, db_obj=db_product, obj_in={"is_active": False})

    def reactivate_product(self, db: Session, product_id: int) -> Optional[Product]:
        db_product = self.repository.get(db, product_id)
        if not db_product:
            return None
        
        return self.repository.update(db, db_obj=db_product, obj_in={"is_active": True})

    def update_stock(self, db: Session, product_id: int, quantity: int) -> Optional[Product]:
        return self.repository.update_stock(db, product_id=product_id, quantity=quantity)

    def check_stock_availability(self, db: Session, product_id: int, required_quantity: int) -> bool:
        product = self.repository.get(db, product_id)
        if not product or not product.is_active:
            return False
        return product.stock >= required_quantity

    def reserve_stock(self, db: Session, product_id: int, quantity: int) -> bool:
        """Reserve stock for an order (decrease stock)"""
        if not self.check_stock_availability(db, product_id, quantity):
            return False
        
        return self.repository.update_stock(db, product_id=product_id, quantity=-quantity) is not None 