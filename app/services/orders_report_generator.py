...
class OrdersReportGenerator:
    ...
    def generate_orders_report(self, orders: List[Order], settings: Settings, output_path: str, 
    ...
    def _create_report_header(self, settings: Settings, title: str, total_orders: int):
        ...
        if settings.logo_url:
            try:
                logo = self._get_logo_image(settings.logo_url, max_width=2.2*cm, max_height=2.2*cm)
                if logo:
                    header_data = [[logo, Paragraph(company_info, self.styles['Normal'])]]
                    header_table = Table(header_data, colWidths=[2.8*cm, 14*cm])
        ...
        elements.append(Paragraph(f'<para align="center">{report_info}</para>', self.styles['CompanyInfo']))
    ...
    def _group_orders_by_client(self, orders: List[Order]) -> Dict:
        ...
        sorted_clients = dict(sorted(orders_by_client.items(), key=lambda x: x[0].name))
    ...
    def _create_client_section(self, client, orders: List[Order]):
        ...
        for order in orders:
            ...
            for item in order.items:
                product_name = item.product.name if len(item.product.name) <= 20 else item.product.name[:17] + "..."
    ...
    def _create_summary_section(self, orders: List[Order]):
        ...
        elements.append(Paragraph("RESUMEN GENERAL", self.styles['ReportTitle']))
        ...
        summary_data.append(['TOTAL GENERAL', f"{total_orders:,}", f"Q {grand_total:,.2f}"])
    ...
    def _get_logo_image(self, logo_url: str, max_width=2.2*cm, max_height=2.2*cm):
