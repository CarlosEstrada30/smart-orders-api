...
class ClientService:
    ...
    async def bulk_upload_clients(
        ...
        # Try to read Excel file - first try "Clientes", then try first sheet
        df = None
        sheet_used = None
        try:
        ...
        if missing_columns:
            ...
            error_msg += f"Required: Column for 'name' (could be: name, nombre, Name, Nombre, NOMBRE, NAME)\n"
            error_msg += f"Make sure your Excel file has a column for the client name."
