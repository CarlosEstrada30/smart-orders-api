...
class FELService:
    ...
    def __init__(self):
        ...
        self.fel_configs = {
            "digifact": FELConfiguration(
                ...
                username=getattr(
                    settings,
                    'FEL_DIGIFACT_USERNAME',
                    'test_user'),
                password=getattr(
                    settings,
                    'FEL_DIGIFACT_PASSWORD',
                    'test_password'),
                nit_empresa=getattr(
                    settings,
                    'COMPANY_NIT',
            ...
            "facturasgt": FELConfiguration(
                ...
                username=getattr(
                    settings,
                    'FEL_FACTURASGT_USERNAME',
                    'test_user'),
                password=getattr(
                    settings,
                    'FEL_FACTURASGT_PASSWORD',
                    'test_password'),
                nit_empresa=getattr(
                    settings,
                    'COMPANY_NIT',
    ...
    def process_fel_authorization(
        ...
        try:
            ...
            # Update FEL fields to track processing without changing main
            # status
            ...
            if fel_response["success"]:
                # Update invoice with FEL data and change status to ISSUED (FEL
                # authorized)
                fel_data = {
                ...
                self.invoice_repository.update(
                    db, db_obj=invoice, obj_in=fel_data)
                # Update status to ISSUED (FEL authorized and ready)
                self.invoice_repository.update_invoice_status(
                    db, invoice_id=invoice_id, status=InvoiceStatus.ISSUED)
                db.commit()
                ...
                self.invoice_repository.update(
                    db, db_obj=invoice, obj_in=error_data)
                db.commit()
            ...
            logger.error(
                f"Error processing FEL for invoice {invoice_id}: {str(e)}")
    ...
    def _generate_fel_xml(self, invoice: Invoice) -> str:
        ...
        receptor.set(
            "IDReceptor",
            invoice.order.client.nit if hasattr(
        ...
        direccion_receptor.set(
            "Direccion",
            invoice.order.client.address or "Ciudad de Guatemala")
        ...
        for idx, item in enumerate(invoice.order.items, 1):
            ...
            codigo_unidad_gravable = ET.SubElement(
                impuesto, "dte:CodigoUnidadGravable")
    ...
    def _send_to_fel_certifier(
            self, fel_xml: str, certifier: str) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.error(
                f"Error sending to FEL certifier {certifier}: {str(e)}")
            return {
                "success": False,
                "error": f"Connection error with certifier: {str(e)}"
            }

    def _send_to_digifact(self, fel_xml: str,
                          config: FELConfiguration) -> Dict[str, Any]:
    ...
    def _send_to_facturasgt(
            self, fel_xml: str, config: FELConfiguration) -> Dict[str, Any]:
    ...
    def retry_failed_fel_processing(
            self, db: Session, certifier: str = "digifact") -> Dict[str, Any]:
        """Retry FEL processing for failed invoices"""

        # Get invoices with FEL errors that need retry (DRAFT status with
        # fel_error_message)
        failed_invoices = db.query(Invoice).filter(
            Invoice.status == InvoiceStatus.DRAFT,
            Invoice.requires_fel,
    ...
    def get_fel_status_summary(self, db: Session) -> Dict[str, Any]:
        ...
        fel_invoices = db.query(
            case(
                ...
                ((Invoice.fel_error_message.isnot(None)) &
                 (Invoice.fel_uuid.is_(None)), "fel_failed"),
        ...
        summary = {
            ...
            "by_status": {
                status.value: count for status,
                count in status_counts},
            "fel_processing": {
                fel_status: count for fel_status,
                count in fel_invoices},
            "processing_needed": 0,
