...
class TenantService:
    ...
    def get_tenants(self, db: Session, skip: int = 0, limit: int = 100, include_inactive: bool = False) -> List[Tenant]:
        ...
        return self.repository.get_active_tenants(db, skip=skip, limit=limit)
    ...
    def delete_tenant(self, db: Session, tenant_id: int) -> Optional[Tenant]:
        """
        Realiza soft delete de un tenant marcándolo como inactivo
        Los datos y el schema se conservan pero el tenant se considera eliminado
        """
        return self.repository.soft_delete(db, id=tenant_id)

    def restore_tenant(self, db: Session, tenant_id: int) -> Optional[Tenant]:
        """
        Restaura un tenant inactivo marcándolo como activo
        """
