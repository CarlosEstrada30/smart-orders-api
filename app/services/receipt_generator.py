...
class ReceiptGenerator:
    ...
    def generate_order_receipt(self, order: Order, company_info: CompanyInfo, output_path: str) -> str:
    ...
    def generate_receipt_buffer(self, order: Order, company_info: CompanyInfo) -> BytesIO:
    ...
    def _draw_receipt(self, c: canvas.Canvas, order: Order, company_info: CompanyInfo):
    ...
    def _draw_header(self, c: canvas.Canvas, order: Order, company_info: CompanyInfo):
        ...
        title_width = c.stringWidth("COMPROBANTE DE PEDIDO", "Helvetica-Bold", 20)
        c.drawString(self.width - self.margin - title_width, y, "COMPROBANTE DE PEDIDO")
    ...
    def _draw_order_details(self, c: canvas.Canvas, order: Order):
        ...
        for i, line in enumerate(details_lines):
            if i == 0:  # Status line with color
                c.drawString(self.margin + 20, y, line)
                status_x = self.margin + 20 + c.stringWidth(line, "Helvetica", 10)
        ...
        if order.notes:
            ...
            c.setFont("Helvetica", 9)
            # Wrap long notes
            note_text = order.notes[:80] + "..." if len(order.notes) > 80 else order.notes
    ...
    def _draw_items_table(self, c: canvas.Canvas, order: Order):
        ...
        for i, item in enumerate(order.items):
            ...
            description = item.product.description[:20] + "..." if item.product.description and len(item.product.description) > 20 else (item.product.description or "N/A")
    ...
    def _draw_footer(self, c: canvas.Canvas, order: Order, company_info: CompanyInfo):
        """Draw footer section"""
        y = 100
