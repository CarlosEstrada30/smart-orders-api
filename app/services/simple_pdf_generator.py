...
class SimplePDFGenerator:
    ...
    def _draw_invoice(self, c: canvas.Canvas, invoice: Invoice, company_info: CompanyInfo):
    ...
    def _draw_header(self, c: canvas.Canvas, invoice: Invoice, company_info: CompanyInfo):
    ...
    def _draw_footer(self, c: canvas.Canvas, invoice: Invoice, company_info: CompanyInfo):
        """Draw footer section"""
        y = 120
        ...
        if invoice.payment_terms:
            c.setFont("Helvetica", 10)
            c.setFillColor(colors.black)
            c.drawString(self.margin, y, f"Términos de Pago: {invoice.payment_terms}")
            y -= 15
