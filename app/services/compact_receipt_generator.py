...
class CompactReceiptGenerator:
    ...
    def generate_order_receipt(self, order: Order, settings: Settings, output_path: str) -> str:
        ...
        story.append(Paragraph("COMPROBANTE DE PEDIDO", self.styles['CompanyTitle']))
        story.append(Spacer(1, 4*mm))
    ...
    def generate_receipt_buffer(self, order: Order, settings: Settings) -> BytesIO:
    ...
    def _create_compact_header(self, settings: Settings):
        ...
        if settings.logo_url:
            try:
                logo = self._get_logo_image(settings.logo_url, max_width=2.5*cm, max_height=2.5*cm)
                if logo:
                    header_data = [[logo, Paragraph(company_info, self.styles['Normal'])]]
        ...
        if contact_parts:
            ...
            elements.append(Paragraph(f'<para align="center">{contact_text}</para>', self.styles['CompanyInfo']))
    ...
    def _create_clean_order_client_info(self, order: Order):
        ...
        order_client_data = [
            ...
            [f"Fecha: {order.created_at.strftime('%d/%m/%Y')}", f"Email: {client.email or 'No proporcionado'}"],
            [f"Hora: {order.created_at.strftime('%H:%M')}", f"Teléfono: {client.phone or 'No proporcionado'}"],
        ...
        info_table.setStyle(TableStyle([
            ...
            ('FONTSIZE', (0, 0), (-1, 0), 10),
            ('TEXTCOLOR', (0, 0), (-1, 0), colors.Color(0.2, 0.2, 0.2)),  # Gris oscuro
            ...
            ('LINEBELOW', (0, 0), (-1, 0), 1, colors.Color(0.5, 0.5, 0.5)),  # Gris medio
        ]))
    ...
    def _create_compact_products_table(self, order: Order):
        ...
        elements.append(Paragraph("PRODUCTOS SOLICITADOS", self.styles['SectionTitle']))
        ...
        for i, item in enumerate(order.items, 1):
            ...
            row = [
                ...
                product_name,
                f"{item.quantity:,}",
                f"Q {item.unit_price:,.2f}",  # Con decimales y espacio después de Q
        ...
        table_style = [
            # Header
            ('BACKGROUND', (0, 0), (-1, 0), colors.Color(0.4, 0.4, 0.4)),  # Gris oscuro
            ...
            ('LINEBELOW', (0, 0), (-1, 0), 2, colors.Color(0.4, 0.4, 0.4)),  # Gris oscuro
    ...
    def _create_compact_total_section(self, order: Order):
        ...
        total_data = [
            ["Total de productos:", f"{len(order.items)} tipos", "Total de unidades:", f"{sum(item.quantity for item in order.items):,}"],
            ["", "", "TOTAL A PAGAR:", f"Q {order.total_amount:,.2f}"]
        ]
        ...
        total_table.setStyle(TableStyle([
            ...
            ('FONTSIZE', (2, 1), (3, 1), 14),
            ('ALIGN', (2, 1), (3, 1), 'RIGHT'),
            ('TEXTCOLOR', (2, 1), (3, 1), colors.Color(0.1, 0.1, 0.1)),  # Negro/gris muy oscuro
            
            # Background para el total
            ('BACKGROUND', (2, 1), (3, 1), colors.Color(0.93, 0.93, 0.93)),  # Gris claro
            ('BOX', (2, 1), (3, 1), 1, colors.Color(0.5, 0.5, 0.5)),  # Borde gris medio
    ...
    def _create_minimal_footer(self, settings: Settings):
        ...
        footer_parts = [f"Generado: {datetime.now().strftime('%d/%m/%Y %H:%M')}"]
        ...
        elements.append(Paragraph(f'<para align="center">{footer_text}</para>', self.styles['CompanyInfo']))
        
        return elements

    def _get_logo_image(self, logo_url: str, max_width=2.5*cm, max_height=2.5*cm):
