...
@router.post("/entries", response_model=InventoryEntryResponse,
...
def get_inventory_entries(
    ...
    try:
        if pending_only:
            return inventory_service.get_pending_entries(
                db, skip=skip, limit=limit)
        elif entry_type:
            entry_type_enum = EntryType(entry_type)
            return inventory_service.get_entries_by_type(
                db, entry_type_enum, skip=skip, limit=limit)
        elif status_filter:
            status_enum = EntryStatus(status_filter)
            return inventory_service.get_entries_by_status(
                db, status_enum, skip=skip, limit=limit)
        elif user_id:
            return inventory_service.get_entries_by_user(
                db, user_id, skip=skip, limit=limit)
        elif product_id:
            return inventory_service.get_entries_by_product(
                db, product_id, skip=skip, limit=limit)
        elif start_date and end_date:
            return inventory_service.get_entries_by_date_range(
...
def get_inventory_summary(
    ...
    return inventory_service.get_entry_summary(
...
@router.get("/entries/number/{entry_number}",
            response_model=InventoryEntryResponse)
...
def create_stock_adjustment(
    ...
    try:
        return inventory_service.create_quick_stock_adjustment(
            db, adjustment, current_user.id)
    except ValueError as e:
        raise HTTPException(status_code=400, detail=str(e))
...
def create_production_entry(
    ...
    try:
        ...
        # Create entry
        created_entry = inventory_service.create_entry(
            db, entry, current_user.id)
        ...
        if auto_complete:
            ...
            # Then complete
            created_entry = inventory_service.complete_entry(
                db, created_entry.id)
...
def create_purchase_entry(
    ...
    try:
        ...
        if auto_approve:
            created_entry = inventory_service.approve_entry(
                db, created_entry.id)
