...
class TenantRepository(BaseRepository[Tenant, TenantCreate, TenantUpdate]):
    ...
    def get_by_token(self, db: Session, *, token: str, include_inactive: bool = False) -> Optional[Tenant]:
        query = db.query(Tenant).filter(Tenant.token == token)
        if not include_inactive:
            query = query.filter(Tenant.active == True)
        return query.first()

    def get_by_subdominio(self, db: Session, *, subdominio: str, include_inactive: bool = False) -> Optional[Tenant]:
        query = db.query(Tenant).filter(Tenant.subdominio == subdominio)
        if not include_inactive:
            query = query.filter(Tenant.active == True)
        return query.first()

    def get_by_schema_name(self, db: Session, *, schema_name: str, include_inactive: bool = False) -> Optional[Tenant]:
    ...
    def get_active_tenants(self, db: Session, skip: int = 0, limit: int = 100) -> List[Tenant]:
        ...
        return db.query(Tenant).filter(Tenant.active == True).offset(skip).limit(limit).all()
