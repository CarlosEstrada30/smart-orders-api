...
class InventoryEntryRepository(
        BaseRepository[InventoryEntry, InventoryEntryCreate, InventoryEntryUpdate]):
    def __init__(self):
        super().__init__(InventoryEntry)

    def get_by_entry_number(self, db: Session, *,
                            entry_number: str) -> Optional[InventoryEntry]:
        return db.query(InventoryEntry).options(
            joinedload(
                InventoryEntry.user), joinedload(
                InventoryEntry.items).joinedload(
                InventoryEntryItem.product)).filter(
                    InventoryEntry.entry_number == entry_number).first()
    ...
    def get_entries_by_type(
            ...
            *,
            ...
            skip: int = 0,
            limit: int = 100) -> List[InventoryEntry]:
        return db.query(InventoryEntry).options(
    ...
    def get_multi(self, db: Session, *, skip: int = 0,
        ...
        return db.query(InventoryEntry).options(
            joinedload(
                InventoryEntry.user), joinedload(
                InventoryEntry.items).joinedload(
                InventoryEntryItem.product)).order_by(
                    desc(
                        InventoryEntry.created_at)).offset(skip).limit(limit).all()
    ...
    def get(self, db: Session, id: int) -> Optional[InventoryEntry]:
        return db.query(InventoryEntry).options(
            ...
            joinedload(
                InventoryEntry.items).joinedload(
                InventoryEntryItem.product)).filter(
                    InventoryEntry.id == id).first()

    def create_entry_with_items(
            self,
            db: Session,
            *,
            entry_data: InventoryEntryCreate,
            user_id: int) -> InventoryEntry:
        ...
        total_cost = sum(
            item.quantity *
            item.unit_cost for item in entry_data.items)
    ...
    def update_entry_status(
            ...
            *,
            entry_id: int,
            status: EntryStatus) -> Optional[InventoryEntry]:
    ...
    def approve_entry(
            ...
            db: Session,
            *,
            entry_id: int) -> Optional[InventoryEntry]:
        """Approve an entry and change status to approved"""
        return self.update_entry_status(
            db, entry_id=entry_id, status=EntryStatus.APPROVED)
    ...
    def get_entry_summary(
        ...
        for entry_type in EntryType:
            count = query.filter(
                InventoryEntry.entry_type == entry_type).count()
