    ...
    def update_pending_order_complete(
        ...
        if order_update.items is not None:
            ...
            for item_data in order_update.items:
                ...
                # Round item values
                item_total_rounded = float(item_total.quantize(Decimal('0.01'), rounding=ROUND_HALF_UP))
                unit_price_rounded = float(Decimal(str(item_data.unit_price)).quantize(Decimal('0.01'), rounding=ROUND_HALF_UP))
