...
class Invoice(Base):
    ...
    fel_uuid = Column(
        String,
        nullable=True,
        index=True)               # UUID de SAT
    dte_number = Column(
        String,
        nullable=True,
        index=True)             # Número DTE
    fel_authorization_date = Column(DateTime(timezone=True), nullable=True)
    # XML generado
    fel_xml_path = Column(String, nullable=True)
    fel_certification_date = Column(DateTime(timezone=True), nullable=True)
    # Certificador usado (ej: FacturasGT, Digifact)
    fel_certifier = Column(String, nullable=True)
    # Serie FEL
    fel_series = Column(String, nullable=True)
    # Número FEL
    fel_number = Column(String, nullable=True)
    # Error FEL si aplica
    fel_error_message = Column(Text, nullable=True)
    # Si requiere procesamiento FEL
    requires_fel = Column(Boolean, default=True)
