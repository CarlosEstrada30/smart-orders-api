...
class InventoryEntry(Base):
    ...
    supplier_info = Column(String, nullable=True)  # Info del proveedor (opcional)
    ...
    expected_date = Column(DateTime(timezone=True), nullable=True)  # Fecha esperada
    completed_date = Column(DateTime(timezone=True), nullable=True)  # Fecha completada
    
    # Additional info
    notes = Column(Text)
    reference_document = Column(String, nullable=True)  # Número de factura, orden de compra, etc.
    ...
    items = relationship("InventoryEntryItem", back_populates="entry", cascade="all, delete-orphan")
...
class InventoryEntryItem(Base):
    ...
    id = Column(Integer, primary_key=True, index=True)
    entry_id = Column(Integer, ForeignKey("inventory_entries.id"), nullable=False)
    ...
    expiry_date = Column(DateTime(timezone=True), nullable=True)  # Fecha de vencimiento
    notes = Column(Text)
